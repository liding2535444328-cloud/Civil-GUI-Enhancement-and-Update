...
%% --- 模块 1.5: 输入特征分布分析 (新增科研图表) ---
figure('Color', [1 1 1], 'Position', [100, 100, 900, 850], 'Name', 'Feature_Distribution');
% 定义对应的英文标签（需与您的 8+1 维度对应）
featureNames_EN = {'Cement', 'Silica Fume', 'Water', 'Superplasticizer', ...
                   'Sand', 'Gravel', 'Curing Age', 'Rubber', 'Strength'};
...
for i = 1:9
    ...
    % 底部双语标注
    xlabel_str = sprintf('%s\n(%s)', allNames{i}, featureNames_EN{i});
...
% 调用避让算法布局总标题
auto_layout_manager(gcf, '图1: 输入特征与强度分布范围分析', 'Fig.1: Data Range Analysis of Input Features and Strength');
...
% 修改为：
text(0.5, -0.32, {'图2: 特征相关性分析'; 'Fig.2: Correlation Analysis'}, ...
