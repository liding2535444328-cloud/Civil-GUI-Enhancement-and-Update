...
% Module 9: Trigger Blind Test Validation
...
%% ========================================================================
%  Module 9: Generalization Blind Test Validation (FA-RF High Compatibility)
%  Function: UI Dialog, SCI Format, Figures Stay Open
% ========================================================================
...
function process_blind_data(filePath, model, modelName, featureNames)
    ...
    if isempty(X_blind)
        errordlg('Error: No valid data found in the selected file.', 'Execution Failed');
        return;
    end

    % --- RF 模型预测 (RF 模型的 predict 返回的是 cell，需要转换) ---
    Y_pred_raw = predict(model, X_blind);
    if iscell(Y_pred_raw)
        Y_pred = str2double(Y_pred_raw);
    else
        Y_pred = Y_pred_raw;
    end
    ...
    scatter(Y_actual, Y_pred, 75, 'filled', 'MarkerFaceColor', [0.47 0.67 0.19], 'MarkerFaceAlpha', 0.6); hold on;
    ...
    stem(Errors, 'filled', 'Color', [0.3 0.3 0.3], 'LineWidth', 1.2); hold on;
    yline(0, 'k-', 'LineWidth', 1.5);
    yline(avg_abs_err, 'r--', 'LineWidth', 1.5);
    yline(-avg_abs_err, 'r--', 'LineWidth', 1.5);
    
    text(length(Errors)*0.5, avg_abs_err + (max(abs(Errors))*0.1), ...
        ['Avg Error: \pm', num2str(avg_abs_err, '%.2f'), ' MPa'], 'Color', 'r', 'FontWeight', 'bold');
    ...
    for i = 1:num_s
        curr_x = X_blind(i, :); 
        p_tmp = predict(model, curr_x);
        if iscell(p_tmp), b_o = str2double(p_tmp); else, b_o = p_tmp; end
        for f = 1:9
            t_x = curr_x; t_x(f) = mean(X_blind(:, f));
            p_f = predict(model, t_x);
            if iscell(p_f), b_f = str2double(p_f); else, b_f = p_f; end
            shap_v(f, i) = b_o - b_f;
        end
    end
    hold on;
    ...
    fprintf('  Worst Error: %.3f MPa\n', max_abs_err);
    
    msgbox({['Analysis Complete for ', modelName], ...
            ['Samples: ', num2str(numDetected)], ...
            ['Avg Error: \pm', num2str(avg_abs_err, '%.2f'), ' MPa']}, 'Success');
end
